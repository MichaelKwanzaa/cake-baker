...
const { userSignInValidation,
    ...
    validateSignIn,
    validateSignUp } = require("../middleware/validator")

//authentication 
//register a user with middle function to check input errors
router.post("/register", userSignUpValidation(), validateSignUp, async (req, res) => {
    const { name, email, password } = req.body;

    //hash current password sent from client
    const hashedPassword = CryptoJS.AES.encrypt(password, process.env.PASSWORD_SECRET);
